"""Per-pair physical-error anomaly detection.

THIS is experimental! Come back to expand asap. Also I need to change 'physical error' to 'model violation' it's a bad name
"""
...
DEFAULT_MODEL_FILENAME = "physical_error_default.joblib"


class PhysicalErrorDetector(Protocol):
    name: str
...
@dataclass
class HistGBMPhysicalErrorDetector:
    ...
    def detect(self, per_pair_features: Any) -> Any:
        try:
            ...
            raise RuntimeError("HistGBMPhysicalErrorDetector requires pandas.") from exc
        if self.estimator is None:
            raise RuntimeError("HistGBMPhysicalErrorDetector has no fitted estimator.")
    ...
    def with_threshold(self, threshold: float) -> "HistGBMPhysicalErrorDetector":
        return HistGBMPhysicalErrorDetector(
            name=self.name,
    ...
    @classmethod
    def default(cls) -> "HistGBMPhysicalErrorDetector":
        """Load the package-bundled default-trained detector."""

        model_path = _default_model_path()
        if not model_path.exists():
            raise FileNotFoundError(
                f"Default physical-error detector model not found at {model_path}. "
                "Train one with scripts/train_physical_error_detector.py."
            )
...
def _default_model_path() -> Path:
    ...
    pkg_root = resources.files("mm_pipeline.qa")
    return Path(str(pkg_root.joinpath("_models", DEFAULT_MODEL_FILENAME)))


def save_detector(detector: HistGBMPhysicalErrorDetector, path: str | Path) -> Path:
...
def load_detector(path: str | Path) -> HistGBMPhysicalErrorDetector:
    ...
    if not isinstance(obj, HistGBMPhysicalErrorDetector):
        raise TypeError(f"Expected HistGBMPhysicalErrorDetector, got {type(obj).__name__}.")
    return obj
...
def build_detector(name: str) -> PhysicalErrorDetector:
    ...
    if name == "hist_gbm_default":
        return HistGBMPhysicalErrorDetector.default()
